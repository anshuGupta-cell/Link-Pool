...
const Modal = (props) => {
    ...
    return (
        <div id="updateHandle" onClick={(e) => { e.target.id === 'updateHandle' && setModalState(false) }} className="fixed top-0 left-0 bottom-0 right-0 bg-black/50 grid">
            <div className="max-w-4xl m-auto rounded-lg  p-3 grid gap-2">
                <form onSubmit={(e) => updateHandle(e)} className="w-72 bg-slate-50 text-slate-700 p-4 mx-auto grid gap-4 focus-within:outline outline-offset-[-.3rem] outline-2 outline-gray-800 rounded-lg ">

                    <div className="font-semibold text-3xl flex justify-between items-center">
                        <h1>Update handle </h1>
                        <img onClick={(e) => { setModalState(false) }} className="w-5" src="/svg/xmark-regular.svg" alt="close" />
                    </div>
                    ...
                    <div className="grid gap-2 ">
                        ...
                        <div className="">
                            <input name="dis" className="text-black rounded-full focus:outline-green-500 py-1 px-3 bg-slate-100" type="text" value={newDesc || ""} onChange={(e) => { setNewDesc(e.target.value) }} placeholder="Enter description" required />
                        </div>
